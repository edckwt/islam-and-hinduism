...
class app_ifh_models {
	...
	{
		global $opicifh_categories_lang;
		$return = array();
		$table = $this->wpdb->prefix.'options';
		$like = OPICIFH_Input_SLUG."category_".get_option(OPICIFH_Input_SLUG.'language').'_';
		...
		if(!empty($data) && is_array($data)){
			foreach ($data as $key => $value) {
				...
				$return[$key]['option_url'] = $opicifh_categories_lang[get_option(OPICIFH_Input_SLUG.'language')][$return[$key]['option_slug']]['importurl'];
